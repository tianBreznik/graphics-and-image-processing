...
void calculate_lookpoint(void) { /* Given an eyepoint and latitude and longitude angles, will
     ...
     //calcualating view direction in cartesian coordinates by converting from
...
void mouse_motion(int x, int y) {
  ...
  //calculate angle based on 2d window mouse coordinates
  mlon = (((width/2.0)-x)*100.0)/width;
  mlat = (((height)/2.0 -y)*100.0)/height;
...
//variable for tracking crouched state
bool crouched = false;
//enum type for tracking state of jump
enum state {JUMPING_UP, JUMPING_DOWN, WALKING};
//initialize global variable current_state to default case - WALKING
enum state current_state = WALKING;
GLfloat before_eyey;
void keyboard(unsigned char key, int x, int y) {
  ...
  switch (key) {
    case 27:  /* Escape key */
      exit(0);
      break;
    case 47: /* Slash key, because it doesn't recognize 67 - c */
      if(!crouched)
      {
        eyey = eyey - 0.5;
        eyex = eyex;
...
void jump(void) {
  // make player jump
  if(current_state == JUMPING_UP)
  {
    eyey = eyey + 0.1;
    centery = centery + 0.1;
    if(eyey > JUMP_LIMIT)
      current_state = JUMPING_DOWN;
  }
  else if (current_state == JUMPING_DOWN)
  {
    //player is in a falling state - lower y eye-coordinate
    eyey = eyey - 0.1;
    centery = centery - 0.1;
    if(eyey < before_eyey)
    {
      //set eyey back to eyey before the jump - player is back on the ground
      eyey = before_eyey;
...
void cursor_keys(int key, int x, int y) {
  //declaration of movement components
  GLfloat D_z;
  GLfloat D_x;
  switch (key) {
    case GLUT_KEY_LEFT:
      //if(abs(lon + TURN_ANGLE) < 90)
      lon = lon + TURN_ANGLE;
      break;
    case GLUT_KEY_RIGHT:
      //if(abs(lon - TURN_ANGLE) < 90)
    ...
    case GLUT_KEY_PAGE_DOWN:
      ...
      break;
    case GLUT_KEY_UP:
      D_z = RUN_SPEED * cos(DEG_TO_RAD*lon);
      D_x = RUN_SPEED * sin(DEG_TO_RAD*lon);
      eyex = eyex + D_x;
      eyez = eyez + D_z;
      eyey = eyey;
      break;
    case GLUT_KEY_DOWN:
      //opposite angle gives negative movement vectors, hence substraction
      D_z = RUN_SPEED * cos(DEG_TO_RAD*lon);
      D_x = RUN_SPEED * sin(DEG_TO_RAD*lon);
      eyex = eyex - D_x;
      eyez = eyez - D_z;
